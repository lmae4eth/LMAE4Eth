...
class BERTContrastivePretraining(nn.Module):
    def __init__(self, model_name, sim='cosine', temperature=0.01, use_contrastive_loss='True'):
        ...
        if self.use_contrastive_loss:
            print ('Initializing anchor BERT.')
            self.anchor_bert = BertModel.from_pretrained(model_name)
            for param in self.anchor_bert.parameters():
                param.requires_grad = False
            print ('anchor BERT initialized.')
        else:
    ...
    def compute_anchor_representations(self, input_ids, token_type_ids, attention_mask):
        bsz, seqlen = input_ids.size()
        outputs = self.anchor_bert(input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)
    ...
    def forward(self, truth, inp, seg, msk, attn_msk, labels, contrastive_labels, nxt_snt_flag):
        ...
        if self.use_contrastive_loss:
            truth_rep, truth_logits, _ =  self.compute_anchor_representations(input_ids=truth, token_type_ids=seg, attention_mask=attn_msk)
            ...
            if self.sim == 'dot_product':
                ...
                contrastive_scores = torch.matmul(masked_rep, truth_rep.transpose(1,2)) / self.temperature
            else:
