...
from dgl.dataloading import DataLoader, LaborSampler


def labor2_sampling_encoder(encoder, g, x, fanouts=[5, 10], batch_size=1024):
    device = x.device
    g = g.to(device)
    sampler = LaborSampler(fanouts)
    ...
    for input_nodes, output_nodes, blocks in dataloader:
        blocks = [b.to(x.device) for b in blocks]
        input_features = x[input_nodes].to(x.device)
        enc_rep = encoder(blocks, input_features)
        enc_reps.append(enc_rep)
    return torch.cat(enc_reps, dim=0)
def labor_sampling_encoder(encoder, g, x, fanouts=[5, 10], batch_size=1024):
    device = x.device
    g = g.to(device)
    sampler = LaborSampler(fanouts)
    dataloader = DataLoader(
    ...
    for input_nodes, output_nodes, blocks in dataloader:
        ...
        input_features = x[input_nodes].to(device)
        enc_rep = encoder(blocks, input_features)
        enc_reps.append(enc_rep)
    return torch.cat(enc_reps, dim=0)
...
class PreModel(nn.Module):
    ...
    def forward(self, g, x, targets=None, epoch=0, drop_g1=None, drop_g2=None):
    ...
    def mask_attr_prediction(self, g, x, targets, epoch, drop_g1=None, drop_g2=None):
        ...
        enc_rep = labor_sampling_encoder(self.encoder, use_g, use_x)
